# ... creates a B-spline basis of degree p with equally spaced elements and the corresponding quadrature grid and weights (bcs = 1: periodic b.c., bcs = 2: Dirichlet b.c.)

from Utils.bsplines import Bspline
import numpy as np

def createBasis(Lz,Nz,p,bcs = 1):
    ...
    if bcs == 1:
        ...
        Tbsp = np.array([0]*p + list(zj) + [Lz]*p)
        bsp = Bspline(Tbsp,p)
        N = len(Tbsp) - p - 1
        xi,wi = np.polynomial.legendre.leggauss(p+1)
        quad_points = np.zeros((p+1)*Nz)
